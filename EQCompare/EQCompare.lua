--[[ $Id: EQCompare.lua 15499 2006-10-30 06:08:12Z hshh $ ]]--
		...
		{ "/eqcompare", "/eqc" },
		{
			...
			args = {
				...
				hoverlink = {
					...
					type = "toggle",
					name = L["hoverlink"],
					desc = L["EQCompare_HOVERLINK_DESC"],
				...
				hoverlink_safe = {
					order = 210,
					type = "toggle",
					name = L["hoverlink safe mode"],
					desc = L["EQCompare_HOVERLINK_SAFE_DESC"],
				...
				merchantauctionkey = {
					order = 220,
					type = "toggle",
					name = L["Merchant/Auction holding key"],
					desc = L["EQCompare_MERCHANTAUCTIONKEY_DESC"],
				...
				hoverlinkkey = {
					order = 230,
					type = "toggle",
					name = L["Hoverlink holding key"],
					desc = L["EQCompare_HOVERLINKKEY_DESC"],
				...
				oskin = {
					order = 300,
					type = "toggle",
					name = L["Turn off oSkin support"],
					desc = L["EQCompare_OSKIN_DESC"],
					get = function() return self.db.profile.oSkin end,
					set = function(c)
						self.db.profile.oSkin = c
					end
				},
				reset = {
					order = 400,
					type = "execute",
					name = L["reset"],
					desc = L["EQCompare_RESET_DESC"],
					func = function()
						self:ResetDB("profile")
						self:HoverHyperlink_Toggle()
						self:SetupOriginHook()
						self:Print(L["EQCompare_RESET"])
					end
				}
			}
		}
...
function EQCompare:Tooltip_OnShow(object)
	...
	for i,invslot in ipairs(slot) do
		...
		if (IsAddOnLoaded("Skinner") and self.db.profile.oSkin) then
